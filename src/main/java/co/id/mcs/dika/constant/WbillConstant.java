...
public class WbillConstant {

    /**
     * Query export wbill untuk data AGREE Credit Card hari ini.
     *
     * Optimasi yang diterapkan:
     * 1. INNER JOIN master_status_result — hasil sama dengan LEFT JOIN + WHERE,
     * tapi lebih jelas bagi query planner sehingga bisa memilih join order terbaik.
     * 2. customer_phone di-pivot dalam satu subquery (GROUP BY) sehingga tabel
     * hanya
     * di-scan SEKALI, bukan 5x LEFT JOIN terpisah.
     * 3. Kondisi filter pada agree_date dan pickup_date_remark menggunakan RANGE
     * (>= dan <) alih-alih DATE(), agar index tetap terpakai.
     * 4. NULLIF pada pickup_time_remark agar TO_CHAR tidak error saat nilai NULL.
     */
    public static final String WBILL_QUERY = """
            WITH phone_pivot AS (
                SELECT
                    customer_id,
            ...
            SELECT 'NoCase' AS c1, 'CIS' AS c2, 'NAME' AS c3, 'DOB' AS c4, 'Gender' AS c5, 'Address_Domisili' AS c6, 'Post_Code_Domisili' AS c7, 'City_Domisili' AS c8,
                   'Phone_Number_1_Home' AS c9, 'Phone_Number_2_Office' AS c10, 'Phone_Number_3_Mobile' AS c11, 'Phone_Number_4_Mobile' AS c12, 'Phone_Number_5_Mobile' AS c13,
                   'ID' AS c14, 'Program_Campaign' AS c15, 'Credit_Card_Type' AS c16, 'Username' AS c17, 'TM_Name' AS c18, 'TM_Code' AS c19, 'Agree_Date' AS c20, 'Agree_Time' AS c21, 'Upload_Date' AS c22,
                   'TGL_PU' AS c23, 'Barcode' AS c24, 'Branch' AS c25, 'PU_Time' AS c26, 'Remarks' AS c27, 'Phone1' AS c28, 'Phone2' AS c29, 'Phone3' AS c30, 'Phone4' AS c31, 'Phone5' AS c32,
                   'HP1' AS c33, 'HP2' AS c34, 'HomePhone1' AS c35, 'HomePhone2' AS c36, 'OfficePhone1' AS c37, 'Area' AS c38,
                   'Nama_Supplement_1' AS c39, 'Hubungan' AS c40, 'Nama_Supplement_2' AS c41, 'Hubungan_2' AS c42, 'Credit_Card_Type_2' AS c43
            UNION ALL
            SELECT * FROM (
                SELECT
                ...
                COALESCE(NULLIF(odd.remark_for_ms, ''), '-')                                AS Remark,
            ...
            FROM order_data od
            INNER JOIN master_status_result msr
                ON od.status_result_id = msr.id AND msr.name = 'AGREE'
            INNER JOIN data_customer_profile cp
                ON od.customer_id = cp.id
            INNER JOIN master_campaign mc
                ON od.campaign_id = mc.id
            INNER JOIN master_campaign_product mcp
                ON mc.id_product = mcp.id AND mcp.product = 'Credit Card'
            LEFT JOIN order_data_details odd
                ON od.id = odd.order_id
            LEFT JOIN master_segment_card msc1
                ON msc1.id = odd.card_segment_1
            LEFT JOIN master_segment_card msc2
                ON msc2.id = odd.card_segment_2
            LEFT JOIN phone_pivot pp
                ON od.customer_id = pp.customer_id
            WHERE od.agree_date >= CURRENT_DATE
              AND od.agree_date  < CURRENT_DATE + INTERVAL '1 day'
              AND (:pAppReferenceId::VARCHAR IS NULL OR od.app_reference_id = :pAppReferenceId)
            ORDER BY od.agree_date ASC
            ) sub
            """;

    private WbillConstant() {
